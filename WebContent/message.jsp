...
<body>
	...
	<s:if test="hasFieldErrors()">
		<s:iterator value="fieldErrors">
			<s:iterator value="value" status="statu">
				...
				<s:set name="msg"
					value="#msg==null?value.get(#request.index).toString()+'\\\n':#msg+   
	...
	<s:if test="hasActionMessages()">
		<s:iterator value="actionMessages">
			<s:iterator value="value" status="statu">
				<!-- field的错误消息是List 因此还需要迭代一次 -->
				<s:set name="index" value="#statu.index" />
				<s:set name="actionMsg"
					value="#actionMsg==null?value.get(#request.index).toString()+'\\\n':#actionMsg+   
                 value.get(#request.index).toString()+'\\\n'" />
				<!-- 关键代码 -->
			</s:iterator>
		</s:iterator>
	...
	<!-- actionmessage -->
	<s:if test="#actionMsg.length()>0">
		<s:iterator value="actionMessages">
			<script language="JavaScript">   
                 alert("<s:property escape="false"  value="#actionMsg"/>");   
             </script>
		</s:iterator>
	</s:if>
