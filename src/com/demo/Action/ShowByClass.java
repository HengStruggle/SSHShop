...
public class ShowByClass extends ActionSupport{	//根据商品种类分页显示商品
	...
	private CommodityClass commodityClazz;
	...
	public CommodityClass getCommodityClazz() {
		return commodityClazz;
	}
	public void setCommodityClazz(CommodityClass commodityClazz) {
		this.commodityClazz = commodityClazz;
	}
	public String execute() throws Exception {
		commodityClasses = commodityClassService.findAllCommodityClass();// 查询所有的商品种类
		commodityClazz = commodityClassService.queryByID(commodityClassID);
		...
		Result result = commodityService.
			queryByCommodityClass(commodityClazz, newPage);//获取查询结果
		page = result.getPage();		//获取分页信息
		commodities = result.getList();	//获取商品信息列表
		
		Map session = ActionContext.getContext().getSession();
		session.put("commoditiesClassName", commodityClazz.getCommodityClassName());
